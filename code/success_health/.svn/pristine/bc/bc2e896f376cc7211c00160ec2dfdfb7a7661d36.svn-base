...
<script>
  ...
  function init_chart()
  {
    chartDivId = 'moodchart';
    datasetNames = ['心情指數', '應對表現'];
    datasets = [[<?php echo join(', ', $chart_values) ?>], [<?php echo join(', ', $chart_performance) ?>]];
    xAxisNames = [<?php echo join(', ', $chart_dates) ?>];
    yAxisTitles = ['( 心情指數 )', '( 應對表現 )'];
    chartTitle = '';
    barMaxNum = 8;
    rightTitleMargin = 70;
  ...
  function recordReflash()
  {
    ...
    $.getJSON("health/mood/functions/analytic/analytic_chart_refresh.php", params).success(function(data) {
      $.fancybox.close();
      datasets = eval("[[" + data.chart_values + "], [" + data.chart_performance + "]]");
