...
<div id="pressurechart" class="log-desc" style="top:40px; left:25px"></div>
...
<script>
  ...
  function init_chart()
  {
    chartDivId = 'pressurechart';
    datasetNames = ['壓力指數'];
    datasets = [<?php echo join(', ', $chart_values) ?>];
    ...
    depositchart( chartDivId, datasetNames, datasets, xAxisNames, yAxisTitles, chartTitle, barMaxNum, false );
  }
  ...
  function recordReflash()
  {
    ...
    $.getJSON("health/pressure/functions/analytic/analytic_chart_refresh.php", params).success(function(data) {
      $.fancybox.close();
      datasets = eval("[" + data.chart_values + "]");
