...
for( $num = 0 ; $num < $dateNum ; $num++ ) {
    ...
    ($unit != 30) ? $rawdates[] = strtotime($_POST['startDate'].' +'.$num*$unit.' day') : $rawdates[] = strtotime( date('Y-m-1', strtotime($_POST['startDate'].' +'.$num.' month')) );

    ($unit != 30) ? $month = date( 'Y/m', strtotime($_POST['startDate'].' +'.$num*$unit.' day') ) : $month = date( 'Y', strtotime($_POST['startDate'].' +'.$num.' month') );
    ($unit != 30) ? $date = date( 'd', strtotime($_POST['startDate'].' +'.$num*$unit.' day') ) : $date = date( 'm', strtotime($_POST['startDate'].' +'.$num.' month') );
...
foreach( $job_dataset as $index => $job ) {
    ...
    $job_dataset[ $index ]['bar'] = array('left' => 0, 'fill' => 0, 'right' => 0);
    $barPadding = 0; // 0:left padding , 1:right padding
    for( $num = 0 ; $num < $dateNum ; $num++ ) {
        if( $unit == 1 ) {
            if( $rawdates[$num] > $jedate ) { 
                break;
            }
            ...
            if( $rawdates[$num] > $jedate ) {
                        //echo "*$num*<br>"; 
                        //echo date('Y-m-1', $rawdates[$num] ).' > '.date('Y-m-d', $jedate ).'<br>';
                        break;
            }
            /*else if( $unit == 30 && strtotime( date('Y-m-1', $rawdates[$num] ) ) > $jedate ) {
                        echo "*$num*<br>"; 
                        break;
            }*/
            if( ($jsdate <= $rawdates[$num] && $rawdates[$num] <= $jedate) || ($jsdate > $rawdates[$num] && $rawdates[$num] <= $jedate ) ) {
                if( $unit == 7 ) {
                    ...
                    for( $i=0 ; $i<7 ; $i++ ) {
                        ( $i % 2 ) ? $paddingWidth = 3 : $paddingWidth = 4;
                        if( $i == 6 ) { $paddingWidth = 3; }
                        if( $jsdate <= $wdates[$i] && $wdates[$i] <= $jedate ) {
                            $job_dataset[ $index ]['barMark'][$num][] = 1;
                            $job_dataset[ $index ]['bar']['fill'] += $paddingWidth;
                            $barPadding = 1;
                        }
                        else{
                            $job_dataset[ $index ]['barMark'][$num][] = 0;
                            if( $barPadding == 0 ) {
                                $job_dataset[ $index ]['bar']['left'] += $paddingWidth;
                            }
                            else if( $barPadding == 1 ) {
                                $job_dataset[ $index ]['bar']['right'] += $paddingWidth; 
                            }
                        }
                    }
                }
                    ...
                    for( $i=0 ; $i<4 ; $i++ ) {
                        if( $jsdate <= $mdates[$i] && $mdates[$i] <= $jedate ) {
                            $job_dataset[ $index ]['barMark'][$num][] = 1;
                            $job_dataset[ $index ]['bar']['fill'] += 6;
                            $barPadding = 1;
                        }
                        else{
                            $job_dataset[ $index ]['barMark'][$num][] = 0;   
                            if( $barPadding == 0 ) {
                                $job_dataset[ $index ]['bar']['left'] += 6;
                            }
                            else if( $barPadding == 1 ) {
                                $job_dataset[ $index ]['bar']['right'] += 6; 
                            }
                        }   
                    }
...
if( count($subjob_dataset) ) {
    foreach( $subjob_dataset as $job_id => $subjobs ) {
        foreach( $subjobs as $index => $subjob ) {
            ...
            $subjob_dataset[$job_id][ $index ]['bar'] = array('left' => 0, 'fill' => 0, 'right' => 0);
            $barPadding = 0; // 0:left padding , 1:right padding
            for( $num = 0 ; $num < $dateNum ; $num++ ) {
                if( $unit == 1 ) {
                    if( $rawdates[$num] > $jedate ) { 
                    ...
                    if( $jsdate <= $rawdates[$num] && $rawdates[$num] <= $jedate ) {
                        ...
                        $subjob_dataset[$job_id][ $index ]['bar']['fill'] += 24;
                        $barPadding = 1;
                    }
                    else {
                        $subjob_dataset[$job_id][ $index ]['barMark'][$num] = 0;
                        if( $barPadding == 0 ) {
                            $subjob_dataset[$job_id][ $index ]['bar']['left'] += 24;
                        }
                        else if( $barPadding == 1 ) {
                            $subjob_dataset[$job_id][ $index ]['bar']['right'] += 24; 
                        }
                    }
                }
                else if( $unit == 7 || $unit == 30 ) {
                    if( ($jsdate <= $rawdates[$num] && $rawdates[$num] <= $jedate) || ($jsdate > $rawdates[$num] && $rawdates[$num] <= $jedate ) ) {
                        if( $unit == 7 ) {
                            ...
                            for( $i=0 ; $i<7 ; $i++ ) {
                                ...
                                if( $jsdate <= $wdates[$i] && $wdates[$i] <= $jedate ) {
                                    $subjob_dataset[$job_id][ $index ]['barMark'][$num][] = 1;
                                    $subjob_dataset[$job_id][ $index ]['bar']['fill'] += $paddingWidth;
                                    $barPadding = 1;
                                }
                                else{
                                    $subjob_dataset[$job_id][ $index ]['barMark'][$num][] = 0;   
                                    if( $barPadding == 0 ) {
                                        $subjob_dataset[$job_id][ $index ]['bar']['left'] += $paddingWidth;
                                    }
                                    else if( $barPadding == 1 ) {
                                        $subjob_dataset[$job_id][ $index ]['bar']['right'] += $paddingWidth; 
                                    }
                                }
                            }
                        }
                            ...
                            for( $i=0 ; $i<4 ; $i++ ) {
                                if( $jsdate <= $mdates[$i] && $mdates[$i] <= $jedate ) {
                                    $subjob_dataset[$job_id][ $index ]['barMark'][$num][] = 1;
                                    $subjob_dataset[$job_id][ $index ]['bar']['fill'] += 6;
                                    $barPadding = 1;
                                }
                                else{
                                    $subjob_dataset[$job_id][ $index ]['barMark'][$num][] = 0;   
                                    if( $barPadding == 0 ) {
                                        $subjob_dataset[$job_id][ $index ]['bar']['left'] += 6;
                                    }
                                    else if( $barPadding == 1 ) {
                                        $subjob_dataset[$job_id][ $index ]['bar']['right'] += 6; 
                                    }
                                }   
                            }
                            /*print_r($job_dataset[ $index ]['barMark'][$num]);
            ...
            /*print_r($subjob_dataset[$job_id][ $index ]['bar']);
            echo '<br>';*/
        }
    }
}
...
?>
<div id="status_list" >
    <div><div class="bar notStarted" style="width: 12px;"></div><div class="status_name inlinebox">未開始</div><div class="bar done" style="width: 12px;"></div><div class="status_name inlinebox">已完成</div></div>
    <div><div class="bar executing" style="width: 12px;"></div><div class="status_name inlinebox">執行中</div><div class="bar delay" style="width: 12px;"></div><div class="status_name inlinebox">延遲</div></div>
</div>
<table cellspacing='0' class="schedule_table data_table center" style="width: 873px; border-radius: 3px 3px 0 0;">
	<thead>
		<tr>
			...
			<td colspan="" style="border-radius: 0 3px 0 0; text-align: right;">
			<?php 
        ...
        <tr>
            ...
            <td class="timeLine" colspan="2"><!--
             ...
             --><div class="inlinebox datesGroup">
                    <div class="month" title="<?php echo $month; ?>" style="<?php if( $countMountNumber == $lastMonthIndex ) echo "border-right:none;"; ?>">
                    <?php 
                    ...
                    foreach( $dates as $date => $day ) {
                        $countDateNumber++;
                    ?>
                     --><div class="date" style="<?php if( $unit == 1 && $day == 0 ) { echo 'color: #FF1C19;'; } if( $countDateNumber == $lastDateIndex ) echo "border-right:none;"; ?>"><?php echo $date; ?></div><!--
                    <?php
                    }
                    ?>
             --></div><!--
...
<table cellspacing='0' class="schedule_table data_table center" style="width: 873px; border-top: none;">
    ...
    <tr>
        ...
        <td style="border-right: none;"><!--
            <?php 
            if( $unit == 1 || $unit == 7 || $unit == 30 ) {
                foreach( $monthEndDate as $barNum ) {
            ?>
             --><div class="inlinebox bar-wrap monthEnd" style="position:absolute; <?php echo 'width:'.(($barNum+1)*24).'px;'; ?>"></div><!--
            <?php
                }
            ?>
             --><div class="bar <?php if($job['bar']['fill'] ) { echo 'fill'; echo ' '.$jobBarColor; } ?>" style="position: relative; <?php echo 'margin-left:'.$job['bar']['left'].'px; width:'.$job['bar']['fill'].'px;'; ?>"></div><!--
            <?php
            }
            /*else if( $unit == 7 ) {
    ...
    <tr>
        ...
        <td style="border-right: none;"><!--
            ...
            ?>
             --><div class="bar <?php if($subjob['bar']['fill'] ) { echo 'fill'; echo ' '.$subjobBarColor; } ?>" style="position: relative; <?php echo 'margin-left:'.$subjob['bar']['left'].'px; width:'.$subjob['bar']['fill'].'px;'; ?>"></div><!--
            <?php
            }
            /*else if( $unit == 7 ) {
    ...
    <tr>
        <td>&nbsp;</td>
        <td style="position: relative; border-right: none;"><!--
        <?php
        if( $unit == 1 || $unit == 7 || $unit == 30 ) {
            foreach( $monthEndDate as $barNum ) {
        ?>
        ...
        ?>
         --><div class="bar" style="position: relative;">&nbsp;</div><!--
        <?php
        }
        ?>
     --></td>
