		...
		return this.each(function(){
			...
			function selected2link()
			{
				if(!toolbar.data("sourceOpened"))
				{
					...
					linktypes.find("a").click(function(e)
					{
						e.preventDefault();
			...
			function selected2style(styleCommand)
			{
				if(!toolbar.data("sourceOpened"))
				{
					...
					styleField.find("a").unbind("click").click(function(e)
					{
						e.preventDefault();
			...
			function selected2format()
			{
				...
				formatbar.find("a").click(function(e)
				{
					e.preventDefault();
			...
			function buttonEmphasize(e)
			{
				for(var n = 0; n < buttons.length; n++)
				{
					if(vars[buttons[n].name] && buttons[n].emphasis && buttons[n].tag!='') {
						...
						if( detectElement(buttons[n].tag) ) {
							toolbar.find('.'+vars.css+'_tool_'+buttons[n].cls).addClass(emphasize);
						}
						else {
							$('.'+vars.css+'_tool_'+buttons[n].cls).removeClass(emphasize);
						}
					}
				}
				// showing text format
