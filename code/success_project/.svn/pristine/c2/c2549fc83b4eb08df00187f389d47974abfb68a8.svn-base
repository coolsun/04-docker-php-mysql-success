...
$profitchart_cost_data = array();
$profitchart_total_value_data = array();
$profitchart_net_profit_data = array();
$profitchart_names = array();


foreach ($lastInvests as $key => $value) {
  $cost = $value['cost'] / 1000;
  $total_value = $value['total_value'] / 1000;
  $net_profit = $value['net_profit'] / 1000;

  array_push($profitchart_cost_data, $cost);
  array_push($profitchart_total_value_data, $total_value);
  array_push($profitchart_net_profit_data, $net_profit);
  array_push($profitchart_names, "'" . $value["name"] . "'");
}
