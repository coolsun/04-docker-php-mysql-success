...
try
{
  $query_string = "SELECT * FROM `invests` WHERE `user_id` = ? AND `account_id` = ? AND `id` = ? LIMIT 1";
  $query = $dbh->prepare($query_string);
  $query->execute( array(
    $uid, $data['aid'], $data['iid']
  ) );
  $thisInvest = $query->fetch( PDO::FETCH_ASSOC );
  // print_r($thisInvest);
  if ($thisInvest['quantity'] != $data['quantity']) {
    $query_string = "SELECT * FROM `invests` WHERE `user_id` = ? AND `account_id` = ? AND `id` <> ? AND `name` = ?";
    $query = $dbh->prepare($query_string);
    $query->execute( array(
    ...
    if ($thisInvest['action'] == 0 && $data['quantity']+$lastQuantity < 0) {
      error('這張股票您修改的買進數量小於目前的賣出數量，請確認後重新輸入!');
  ...
  $query_parts = array();
  $params = array();
  $update_query_string = "UPDATE `invests` SET ";

  if ( isset($data["price"]) )
  {
      // $update_query_string .= " `price`=?";
      $query_parts[] = " `price`=?";
      $params[] = $data["price"];
  }
  if ( isset($data["quantity"]) )
  {
      // $update_query_string .= " `quantity`=?";
      $query_parts[] = " `quantity`=?";
      $params[] = $data["quantity"];
  }
  if ( isset($data["fee"]) )
  {
      // $update_query_string .= " `fee`=?";
      $query_parts[] = " `fee`=?";
      $params[] = $data["fee"];
  }
  if (count($params) == 0) {
  ...
  $update_query_string .= join(",", $query_parts) . " ";
  ...
  $update_query_string .= " WHERE `user_id`=? AND `account_id`=? AND `id`=?";
  // echo $update_query_string . "\n";
  ...
  $originMoney = ($thisInvest['price'] * $thisInvest['quantity']);
  if ( $thisInvest['action'] == 0 )
  {
      $originMoney = -$originMoney - $thisInvest['fee'];
  }
  else if ( $thisInvest['action'] == 1 )
  {
      $originMoney = $originMoney - $thisInvest['fee'];
  ...
  if ( $has_monthly_record !== false  ) // If has monthly account reocrd, update that account and accounts monthly after the bill date
	{
		...
		$update_query_string = "UPDATE `accounts_monthly` SET `account_balance`=`account_balance`+".$diffMoney.", `updated`=? WHERE `user_id`=? AND `account_id`=? AND `updated_date`>=?";
		$update_query = $dbh->prepare( $update_query_string );
		$update_query->execute( array($datetime, $uid, $data['aid'], $year_month.'-01') );

		$update_query_string = "UPDATE `accounts` SET `account_balance`=`account_balance`+".$diffMoney.", `updated`=? WHERE `user_id`=? AND `account_id`=?";
		$update_query = $dbh->prepare( $update_query_string );
